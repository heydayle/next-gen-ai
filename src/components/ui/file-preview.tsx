      ...
      >
        <div className="flex w-full items-center space-x-2">
          ...
          <img
            ...
            className="bg-muted grid size-10 shrink-0 place-items-center rounded-sm border object-cover"
        ...
        {onRemove ? (
          <button
            className="bg-background absolute -right-2 -top-2 flex size-4 items-center justify-center rounded-full border"
            type="button"
            onClick={onRemove}
...
const TextFilePreview = React.forwardRef<HTMLDivElement, FilePreviewProps>(
  ({ file, onRemove }, ref) => {
    ...
    return (
      ...
      >
        <div className="flex w-full items-center space-x-2">
          <div className="bg-muted grid size-10 shrink-0 place-items-center rounded-sm border p-0.5">
            <div className="text-muted-foreground size-full overflow-hidden text-[6px] leading-none">
              {preview || 'Loading...'}
            </div>
          </div>
          <span className="text-muted-foreground w-full truncate">
...
const GenericFilePreview = React.forwardRef<HTMLDivElement, FilePreviewProps>(
  ({ file, onRemove }, ref) => {
    return (
      ...
      >
        <div className="flex w-full items-center space-x-2">
          <div className="bg-muted grid size-10 shrink-0 place-items-center rounded-sm border">
            <FileIcon className="text-foreground size-6" />
          </div>
          <span className="text-muted-foreground w-full truncate">
            {file.name}
          </span>
